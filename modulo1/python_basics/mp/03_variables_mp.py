ticket_id = 'TKT-Q01'
tickets_actuales = 450
prioridad_estandar = 0.45
sistema_activo = True
registro_incidencias = None

print(type(ticket_id))
print(type(tickets_actuales))
print(type(prioridad_estandar))
print(type(sistema_activo))
print(type(registro_incidencias))

a, b, c = 10, 20, 30
print(a)
print(b)
print(c)

a = b = c = 0
print(a)
print(b)
print(c)

codigo_ticket, codigo_respaldo = 101, 202
print(codigo_ticket, codigo_respaldo)
codigo_ticket, codigo_respaldo = codigo_respaldo, codigo_ticket
print(codigo_ticket, codigo_respaldo)

servidor_norte = "Servidor Carcelén"
ServidorNorte = "Servidor Carcelén"
CAPACIDAD_MAXIMA = 1500
_token_seguridad = "XY-99"

id_incidencia = 505
saldo_negativo = -2
presupuesto_infraestructura = 2_500_000_000
combinaciones_red = 5 ** 80
print(id_incidencia)
print(saldo_negativo)
print(presupuesto_infraestructura)
print(combinaciones_red)

sector_bin = 0b1111
sector_oct = 0o25
sector_hex = 0xAC
print(sector_bin, sector_oct, sector_hex)

print(bin(1024))
print(oct(1024))
print(hex(1024))